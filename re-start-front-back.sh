...
sudo docker run  -it --gpus all -p 3000:3000 \
  --mount type=bind,source=./lang_packs,target=/app/lang_packs \
  --mount type=bind,source=./whisper_files,target=/root/.cache/huggingface/hub/ \
  --restart=on-failure:10 --name ctr_neuta_conference_backend -d img_neuta_conference_backend 
else
echo "========================= Ok ==========================================================="
fi
